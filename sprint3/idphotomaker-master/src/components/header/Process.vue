...
<template>
    <Modal v-model="showModal" draggable sticky :reset-drag-position="true" :mask="false" :width="600" title="图片处理"
        ...
        <div>
            <Row :gutter="10">
                <Col :span="12">
                <canvas id="d" ref="d" class="right" width="280" height="400"></canvas>
                </Col>

                <Col :span="12">
                <div>亮度</div>
                <Slider v-model="brightness" :min="-100" :max="100" @on-change="brightnessChange" :disabled="!obj"></Slider>
                <div>模糊</div>
                <Slider v-model="blur" :min="0" :max="100" @on-change="blurChange" :disabled="!obj"></Slider>
                <div>对比度</div>
                <Slider v-model="contrast" :min="-100" :max="100" @on-change="contrastChange" :disabled="!obj"></Slider>
                <div>噪声</div>
                <Slider v-model="noise" :min="0" :max="1000" @on-change="noiseChange" :disabled="!obj"></Slider>
                <div>色调</div>
                <Slider v-model="hue" :min="-100" :max="100" @on-change="hueChange" :disabled="!obj"></Slider>
                </Col>
            </Row>
        </div>
...
export default {
    ...
    mounted() {
        ...
        var canvas = this.__canvas = new fabric.Canvas("d"); //cavans
        canvas.on({
            'selection:created': () => {
