...
export default {
  ...
  methods: {
    ...
    async replaceBgColor(color) {
      const blob = this.dataURItoBlob(this.getCurrentImgSrc);
      const imageData = await getImageData(this.getCurrentImgSrc);
      
      console.log(imageData[0][0])
      const result = await useKmeans(imageData[0]);
      console.log(result)
      const target = result.find(x => x.clusterInd.includes(0));

      const base64Str = toBase64(target,200,280)
      let bgColor = ''
      // switch (color) {
      //   case 'blue':
      //     bgColor = '#00BFF3'
      ...
      //pconst frontImgSrc = `data:image/png;base64,${frontImgBase64}`
      const frontImg = new Image()
      frontImg.src = base64Str
      frontImg.width = 200
      frontImg.height = 280
      frontImg.onload = () => {
        const canvas = this.$refs.canvas
